...
export const grantCategoryAccess = async (req, res) => {
    ...
    if (!access_level) {
        throw new BadRequestError('access_level is required');
    }

    // Validate access level
    const validAccessLevels = ['view', 'edit', 'assign'];
    if (!validAccessLevels.includes(access_level)) {
        throw new BadRequestError(`Invalid access level. Must be one of: ${validAccessLevels.join(', ')}`);
    }
...
export const revokeCategoryAccess = async (req, res) => {
    ...
    if (isNaN(categoryId) || isNaN(accessId)) {
        throw new BadRequestError('Invalid category ID or access ID');
    }
    ...
    if (accessCheck.rows.length === 0) {
        throw new NotFoundError('Access record not found or does not belong to this category');
    }
...
export const getUserCategoryAccess = async (req, res) => {
    ...
    if (req.user.id !== userId && req.user.role !== 'admin') {
        throw new ForbiddenError('Access denied. Cannot view other users category access.');
    }
