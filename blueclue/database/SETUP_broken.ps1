...
psql -U postgres -c "DROP DATABASE IF EXISTS blueclue;" 2>&1 | Out-Null
...
$authResult = psql -U postgres -d blueclue -f auth_setup.sql 2>&1 | Out-String
...
if (-not $SkipSeed) {
    ...
    $seedResult = psql -U postgres -d blueclue -f seed.sql -q 2>&1 | Out-String
...
$counts = psql -U postgres -d blueclue -t -A -c "SELECT COUNT(*) FROM users WHERE role = 'technician'; SELECT COUNT(*) FROM users WHERE role = 'customer'; SELECT COUNT(*) FROM users WHERE role = 'admin'; SELECT COUNT(*) FROM categories; SELECT COUNT(*) FROM tickets;" 2>&1
